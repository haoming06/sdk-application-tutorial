...
func GetCmdSetTel(cdc *codec.Codec) *cobra.Command {
	return &cobra.Command{
		Use:   "set-tel [name] [tel]",
		Short: "set whois tel",
		Args:  cobra.ExactArgs(2),
		RunE: func(cmd *cobra.Command, args []string) error {
			cliCtx := context.NewCLIContext().WithCodec(cdc).WithAccountDecoder(cdc)
			...
			msg := nameservice.NewMsgSetTel(args[0], args[1], cliCtx.GetFromAddress())(args[0], coins, cliCtx.GetFromAddress())
			err = msg.ValidateBasic()
			if err != nil {
				return err
			}
